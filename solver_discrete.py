...
class Solver:
    ...
    def __init__(
        ...
        self.possible_connections = list(itertools.combinations(range(self.n_points), 2))
        self.xy_points = np.array([self._get_coordinates(s) for s in self.s_vals])
        # adjecency matrix, uses only upper triangle
    ...
    def score(self, sidx1, sidx2):
        return self._target_function(self.xy_points[sidx1], self.xy_points[sidx2]) + self._closeness_penalty(self.s_vals[sidx1], self.s_vals[sidx2])
    ...
    def solve_next(self):
        ...
        scores = [f(self.xy_points[idx1], self.xy_points[idx2]) + self._closeness_penalty(self.s_vals[idx1], self.s_vals[idx2]) for idx1, idx2 in self.possible_connections]
        # self._target_function = self._build_target_function()
        # scores = [self.score(idx1, idx2) for idx1, idx2 in self.possible_connections]
        # with Pool(4) as pool:
        #     scores = pool.starmap(self.score, self.possible_connections, chunksize=2000)

        best_conn_idx = np.argmin(scores)
        best_fval = scores[best_conn_idx]
        best_sidxs = self.possible_connections[best_conn_idx]

        self.string_count += 1
        self.possible_connections.pop(best_conn_idx)
        print(
        ...
        self._img = self._update_img(*best_sidxs, self._img, opacity=self.opacity)
        return (*best_sidxs,)

    def _update_img(self, sidx1, sidx2, img, opacity=None):
        if opacity is None:
...
class SolverGUI(Solver):
    ...
    def update_img_canvas(self, s1, s2):
        self._img_canvas = self._update_img(
...
if __name__ == "__main__":
    ...
    img = cv2.imread(
        r"C:\Users\CapDaniels\Meine Ablage\Documents\CodingProjects\pythonProjects\string_art\test_images\Snoopy_Peanuts.png"
    )
    solver = SolverGUI(circle, img, line_thickness=0.2, dpmm=6.0, n_points=300)
    solver.start_gui()
    # solver.solve_next()
